...
function Form({register , Link , openModal}) {
  ...
  return (
    <form className="form" onSubmit={handleSubmit}>
        <h1>{register?"Register":"Login to your account"}</h1>
      ...
      <div className="inputForm">
        ...
        <input ref={emailRef} type="email" className="input" placeholder="Enter your Email" required />
      </div>
      ...
      <button className="button-submit center">
        {
          load?<Loaders Link={Link} color={'#fff'} /> :(!register?'Sign In':'Sign Up')
        }
      ...
      <p className="p">
        {
          !register?<> Don&asp;t have an account? <span className="span"><Link to={'/register'}>Sign Up</Link></span></>:<>Already have an account? <span className="span"><Link to={'/login'}>Sign in</Link></span></>
        }
